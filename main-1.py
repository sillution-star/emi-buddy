...
from engine import calculate_emi, find_effective_rate, flat_rate, get_amortization, generate_insights, bullet_payment, balloon_payment, compare_loans
import io
import os
...
# --- BULLET PAYMENT ---
@app.post("/bullet")
def calc_bullet(loan_amount: float, annual_rate: float, tenure_months: int, bullet_amount: float, bullet_month: int):
    result = bullet_payment(loan_amount, annual_rate, tenure_months, bullet_amount, bullet_month)
    return result


# --- BALLOON PAYMENT ---
@app.post("/balloon")
def calc_balloon(loan_amount: float, annual_rate: float, tenure_months: int, balloon_amount: float):
    result = balloon_payment(loan_amount, annual_rate, tenure_months, balloon_amount)
    return result


# --- LOAN COMPARISON ---
@app.post("/compare")
def compare(l1_principal: float, l1_rate: float, l1_tenure: int, l1_rate_type: str, l1_name: str, l2_principal: float, l2_rate: float, l2_tenure: int, l2_rate_type: str, l2_name: str):
    loan1 = {"principal": l1_principal, "annual_rate": l1_rate, "tenure_months": l1_tenure, "rate_type": l1_rate_type, "name": l1_name}
    loan2 = {"principal": l2_principal, "annual_rate": l2_rate, "tenure_months": l2_tenure, "rate_type": l2_rate_type, "name": l2_name}
